...
$this->load->library('form_validation');
	...
	function gerenciar(){
		...
		$this->data['results'] = $this->os_model->getOs('os','idOs,dataInicial,dataFinal,hora,descricaoProduto,defeito,status,observacoes,laudoTecnico',$where_array,$config['per_page'],$this->uri->segment(3));
    ...
    public function myformAjax($id) {
       $result = $this->db->where("idUsuarios",$id)->get("usuarios")->result();
   ...
   exit();
    ...
    public function listausuarios(){
         $event = $this->os_model->listausuariosagenda();
        $obj =NULL;
         foreach ($event as $i) {
    $obj[] = [
        'id' => $i['idUsuarios'],
...
$usuarios = $this->Os_model->retorna_usuarios();

$option = "<option value=''></option>";
foreach($usuarios -> result() as $linha) {
$option .= "<option value='$linha->idUsuarios'>$linha->nome</option>";
}

$variaveis['options_usuarios'] = $option;

$this->load->view('editarOs', $variaveis);
  ...
  function editar() {
    ...
    function excluir(){
        ...
        if(!$this->permission->checkPermission($this->session->userdata('permissao'),'dOs')){
           $this->session->set_flashdata('error','Você não tem permissão para excluir agendamento.');
           redirect(base_url());
        }
  ...
  public function adicionarServico(){
        ...
        $data = array(
            ...
            'idUsuarios_os'=> $this->input->post('usuarios'),
            'horaservico_os'=>$this->input->post('hora'),
